...
// 所有字母都是小写的，所有输入都是有效的。
var height = function (sentence) {
    var wordList = sentence.split(' ');
    var all = wordList.map(function (item, index) {
        return {
            count: item.toUpperCase().split('').reduce(function (p, i) {
                return p + i.charCodeAt(0) - 64;
            }, 0),
            world: item
        };
    });
    return all.sort(function (a, b) { return b.count - a.count; })[0].world;
};
